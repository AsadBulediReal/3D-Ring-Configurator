...
function TopStarCurved({
  ...
  const textAngle = (angle, adjustmentFactor) => {
    let value;
    if (totallTextArray.length >= 6) {
      return (value = [0.1, -0.8, adjustmentFactor * angle]);
    } else if( totallTextArray.length >= 3 || totallTextArray.length < 5){
      return (value = [-0.2, -0.8, adjustmentFactor * angle])
    } else  {
      // return (value = [
    ...
    <group position={[0, 0.25 * flipped, 14.85]} scale={[-1, flipped, 1.2]}>
      ...
      {textArray.map((char, index) => {
        ...
        if (
          ...
          mesh.position.set(posi[0] + -0.5, posi[1] + -0.5, posi[2] + 0.5);
        } else if (totallTextArray.length === 2) {
          mesh.position.set(posi[0] + -0.3, posi[1] + -0.2, posi[2] + 0.4);
        } else if (totallTextArray.length === 3) {
          mesh.position.set(posi[0] + -0.3, posi[1] + -0.2, posi[2] + 0.5);
        } else if (totallTextArray.length === 4) {
          mesh.position.set(posi[0] + -0.3, posi[1] + -0.5, posi[2] + 0.5);
        } else if (totallTextArray.length === 5) {
          mesh.position.set(posi[0] + -0.4, posi[1] + -0.8, posi[2] + 0.5);
        } else if (totallTextArray.length > 5 && totallTextArray.length < 8) {
          mesh.position.set(posi[0] - 0, posi[1] + -0.5, posi[2] + 0.9);
        } else if (totallTextArray.length >= 8) {
          mesh.position.set(posi[0] + 0.15, posi[1] + -0.85, posi[2] + 0.9);
        }
        mesh.scale.set(fontScale * flipped, -fontScale, 1); // Apply dynamic scale
        function getAngleAdjustment(index, numCharacters, char) {
          // Define a threshold (adjust this value as needed)
